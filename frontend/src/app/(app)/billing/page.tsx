...
function Pill({
  ...
  tone: "blue" | "amber" | "emerald" | "rose" | "yellow";
...
export default function BillingPage() {
    ...
    try {
      ...
      const results = await Promise.allSettled(
        (projects || []).map(async (p) => {
          const r = await api<{ ok: true; quotes: QuoteApiRow[] }>(`/projects/${p.id}/quotes`);
          const list = r?.quotes || [];
  ...
  const cobrableQuotes = useMemo(() => {
    const completed = quotes.filter((q) => String(q.status).toLowerCase() === "completed");
  ...
  function generateInvoiceFromAgreement() {
    ...
    const newInvoice: InvoiceRow = {
      id: String(Date.now()),
      client: invoiceAgreement.client || invoiceAgreement.client_email || "Cliente",
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-white">
        ...
        <div className="max-w-[900px]">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              ...
              <h1
                className="mt-2 text-[26px] font-black !text-white"
                style={{
                  color: "#ffffff",
              ...
              <p className="text-sm !text-slate-300">Gestiona y rastrea tus cobros</p>
            </div>
          </div>
          ...
          <div className="mb-6 flex flex-wrap gap-3">
            ...
            <button
              type="button"
              onClick={openAttach}
              className="rounded-2xl border border-white/8 bg-white/[0.06] px-5 py-3 text-sm font-bold text-slate-200 hover:bg-white/[0.1]"
            >
              Adjuntar Documento
            </button>
          </div>
          ...
          <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-4">
            <button
              ...
              className={cx(
                ...
                invoiceStatusFilter === "all"
                  ? "text-[#0b0f17] border-transparent"
            ...
            >
              <div
                className={cx(
                  ...
                  invoiceStatusFilter === "all" ? "text-[#0b0f17]/80" : "text-slate-400"
                )}
              >
                Todos
              </div>
            ...
            >
              <div
                className={cx(
                  "text-xs font-extrabold",
                  invoiceStatusFilter === "pending" ? "text-white/80" : "text-slate-400"
                )}
              >
                Pendiente
              </div>
            ...
            >
              <div
                className={cx(
                  "text-xs font-extrabold",
                  invoiceStatusFilter === "overdue" ? "text-white/80" : "text-slate-400"
                )}
              >
                Vencido
              </div>
            ...
            >
              <div
                className={cx(
                  "text-xs font-extrabold",
                  invoiceStatusFilter === "paid" ? "text-white/80" : "text-slate-400"
                )}
              >
                Pagado
              </div>
          ...
          {loading ? (
            ...
            <div className="rounded-3xl border border-white/8 bg-[#0d1320] p-10 text-center">
              ...
              <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
                <button
                  type="button"
                  onClick={openAttach}
                  className="rounded-2xl border border-white/8 bg-white/[0.06] px-6 py-3 text-sm font-bold text-slate-200 hover:bg-white/[0.1]"
                >
                  Adjuntar Documento
                </button>
                ...
                <button
                  type="button"
                  onClick={openCreate}
                  className="rounded-2xl px-6 py-3 text-sm font-extrabold text-[#0b0f17]"
                  style={{ background: "linear-gradient(135deg,#f2c94c,#d4a72c)" }}
                >
                  Crear Documento
                </button>
            ...
            <div className="rounded-3xl border border-white/8 bg-[#0d1320] p-10 text-center">
              <div className="text-sm text-slate-300">No hay documentos para este estado</div>
              <button
                type="button"
                onClick={() => setInvoiceStatusFilter("all")}
                className="mt-4 rounded-2xl px-5 py-2.5 text-sm font-bold text-[#0b0f17]"
                style={{ background: "linear-gradient(135deg,#f2c94c,#d4a72c)" }}
              >
                Ver todos
              </button>
            ...
            <div className="flex flex-col gap-3">
              {filteredInvoices.map((inv) => (
                <div key={inv.id} className="rounded-3xl border border-white/8 bg-[#0d1320] p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-xs text-slate-400">
                        {inv.isAttached
                          ? `Adjunto: ${inv.fileName || "-"}`
                          : `Cotización: ${inv.quoteId || "-"}`}
                      </div>
                      <div className="mt-2 text-xs text-slate-500">Fecha: {inv.date}</div>
                    </div>
      ...
      <ModalShell open={showCreateInvoiceModal} onClose={() => setShowCreateInvoiceModal(false)}>
        <div className="w-[95%] max-w-[680px] overflow-hidden rounded-[24px] border border-white/10 bg-[#0d1320] shadow-2xl">
          <div className="px-8 pb-5 pt-8">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#f2c94c]/12 border border-[#f2c94c]/20 text-[#f2c94c]">
                <span className="text-lg">💳</span>
              </div>
              <div>
                <div className="text-[20px] font-extrabold text-white">Crear Documento de Cobro</div>
            ...
            <div className="mt-5 rounded-2xl border border-blue-500/20 bg-blue-500/10 px-5 py-4 text-sm text-blue-300">
            ...
            <div className="mt-6">
              <div className="text-sm font-bold text-slate-200">Seleccionar Cotización *</div>

              {quotesLoading || !quotesLoaded ? (
                <div className="mt-4 text-sm text-slate-400">Cargando cotizaciones...</div>
              ) : cobrableQuotes.length === 0 ? (
                <div className="mt-4 rounded-2xl border border-white/8 bg-white/[0.03] p-10 text-center">
                  <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-2xl border border-white/8 bg-white/[0.04] text-[#f2c94c]">
                  ...
                  <button
                    ...
                    className="mt-5 inline-flex items-center justify-center rounded-2xl px-5 py-2.5 text-sm font-extrabold text-[#0b0f17]"
                    style={{ background: "linear-gradient(135deg,#f2c94c,#d4a72c)" }}
                  >
                    + Crear Cotización
                  </button>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-2 flex items-center justify-between">
                          ...
                          <div className="text-sm font-extrabold text-[#f2c94c]">
            ...
            {invoiceAgreement ? (
              <div className="mt-6 grid gap-4">
                <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-5 py-4">
                  <div className="mb-1 text-xs font-extrabold text-emerald-300">Cotización seleccionada</div>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-white/8 bg-white/[0.04] px-4 py-3 text-sm text-white outline-none focus:bg-white/[0.06] focus:ring-2 focus:ring-[#f2c94c]/20"
                    />
                  </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full resize-none rounded-2xl border border-white/8 bg-white/[0.04] px-4 py-3 text-sm text-white outline-none focus:bg-white/[0.06] focus:ring-2 focus:ring-[#f2c94c]/20"
                  />
                </div>
              </div>
            ) : null}
          ...
          <div className="flex gap-3 px-8 pb-8 pt-4">
            <button
              ...
              className="flex-1 rounded-2xl bg-white/[0.06] px-5 py-3 font-semibold text-slate-200"
            >
              Cancelar
            </button>
            <button
              ...
              className="flex-1 rounded-2xl px-5 py-3 font-extrabold text-[#0b0f17] disabled:opacity-60"
              style={{ background: "linear-gradient(135deg,#f2c94c,#d4a72c)" }}
            >
              Generar Documento
            </button>
      ...
      <ModalShell open={showInvoicePreview && !!generatedInvoice} onClose={() => setShowInvoicePreview(false)}>
        <div className="flex max-h-[90vh] w-full max-w-[800px] flex-col overflow-hidden rounded-[20px] border border-white/10 bg-[#0d1320] shadow-2xl">
          <div className="border-b border-white/8 px-6 py-5">
            <div className="text-lg font-extrabold text-white">Vista previa del documento</div>
            <div className="mt-1 text-xs text-slate-400">Revisa antes de guardar</div>
          </div>
          ...
          <div className="flex gap-3 border-t border-white/8 px-6 py-5">
            <button
              type="button"
              onClick={() => setShowInvoicePreview(false)}
              className="flex-1 rounded-xl bg-white/[0.06] px-4 py-3 font-semibold text-slate-200"
            >
              Volver
            </button>
            <button
              ...
              className="flex-1 rounded-xl px-4 py-3 font-extrabold text-[#0b0f17]"
              style={{ background: "linear-gradient(135deg,#f2c94c,#d4a72c)" }}
            >
              Guardar Documento
            </button>
      ...
      <ModalShell open={showAttachModal} onClose={() => setShowAttachModal(false)}>
        <div className="w-[90%] max-w-[480px] rounded-[20px] border border-white/10 bg-[#0d1320] p-7 shadow-2xl">
          <h3 className="mb-2 text-[20px] font-extrabold text-white">Adjuntar Documento</h3>
          <p className="mb-6 text-sm text-slate-400">Sube un documento de cobro existente</p>

          <div className="flex flex-col gap-4">
            <div
              className="cursor-pointer rounded-xl border-2 border-dashed border-white/10 bg-white/[0.03] p-8 text-center"
            ...
            >
              ...
              {attachedFileName ? (
                <div>
                  ...
                  <div className="mb-1 font-semibold text-emerald-300">{attachedFileName}</div>
                  <div className="text-xs text-slate-500">Click para cambiar archivo</div>
                </div>
              ) : (
                <div>
                  <div className="mb-1 text-sm font-extrabold text-white">Seleccionar archivo</div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-white/8 bg-white/[0.04] px-4 py-3 text-sm text-white outline-none focus:bg-white/[0.06] focus:ring-2 focus:ring-[#f2c94c]/20"
              />
            </div>
          ...
          <div className="mt-6 flex gap-3">
            ...
            <button
              ...
              className={cx(
                "flex-1 rounded-xl px-4 py-3 font-extrabold text-[#0b0f17]",
