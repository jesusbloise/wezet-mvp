...
function Avatar({ name, type }: { name: string; type: ContactType }) {
  ...
  return (
    <div
      className={cx(
        "flex h-11 w-11 items-center justify-center rounded-2xl border font-black",
        bg
      )}
    >
      {letter}
...
export default function ContactsPage() {
  ...
  async function saveContact() {
    ...
    try {
      ...
      let r: { ok: true; contact: ContactRow } | undefined;

      if (isEditing && editingId) {
        r = await api<{ ok: true; contact: ContactRow }>(`/contacts/${editingId}`, {
          method: "PATCH",
          body: JSON.stringify(payload),
        });
      } else {
        r = await api<{ ok: true; contact: ContactRow }>("/contacts", {
          method: "POST",
          body: JSON.stringify(payload),
        });
      }
      ...
      const updatedContact = r?.contact || null;
  ...
  async function deleteContact(id: string) {
    ...
    setErr(null);
    try {
      await api<{ ok: true; deleted: boolean }>(`/contacts/${id}`, {
        method: "DELETE",
      });
      await loadContacts();
      setSelected(null);
      ...
      <main className="px-5 py-8 sm:px-8">
        <div className="max-w-[980px]">
          <div className="flex items-start justify-between gap-3">
            <div>
  <div className="font-ui text-[11px] uppercase tracking-[0.18em] text-slate-200">
    ...
    Tus contactos guardados
          ...
          <div className="mt-6">
            <div className="relative">
              ...
              />
              <div className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-300">
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-[1fr_360px]">
            <div className="overflow-hidden rounded-3xl border border-white/8 bg-[#0d1320]">
              <div className="border-b border-white/8 bg-[#111827] px-6 py-5">
                ...
                <div className="text-xs text-slate-300">Contactos persistidos en la BD</div>
              </div>
              ...
              <div className="px-6 py-6">
                {loading ? (
                  ...
                  <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] p-10 text-center">
                    <div className="text-sm font-semibold text-slate-200">No hay contactos</div>
                    <div className="mt-1 text-xs text-slate-300">Crea tu primer contacto</div>
                  </div>
                ) : (
            ...
            <div className="overflow-hidden rounded-3xl border border-white/8 bg-[#0d1320]">
              <div className="border-b border-white/8 bg-[#111827] px-6 py-5">
                ...
                <div className="text-xs text-slate-300">Acciones</div>
              </div>

              <div className="px-6 py-6">
                {!selected ? (
                  <div className="text-sm text-slate-300">Selecciona un contacto.</div>
                ) : (
                  <div>
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {selected.phone ? (
                          <div className="mt-1 text-xs text-slate-300">{selected.phone}</div>
                        ) : null}
                      </div>
                    </div>
                    ...
                    {selected.specialty ? (
                      <div className="mt-5 rounded-2xl border border-white/8 bg-white/[0.03] p-4">
                        ...
                        <div className="mt-2 text-sm text-slate-200">{selected.specialty}</div>
                      </div>
                    ) : null}
                    ...
                    {selected.company ? (
                      <div className="mt-5 rounded-2xl border border-white/8 bg-white/[0.03] p-4">
                        ...
                        <div className="mt-2 text-sm text-slate-200">{selected.company}</div>
                      </div>
                    ) : null}
                    ...
                    <div className="mt-5 grid gap-2">
                      <button
                        ...
                        className="rounded-2xl border border-[#f2c94c]/20 bg-[#f2c94c]/10 px-5 py-3 text-sm font-bold text-[#f2c94c] hover:bg-[#f2c94c]/15"
                      >
                        Editar
                      </button>
      ...
      >
        <div className="grid gap-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              <div className="text-xs font-bold text-slate-300">Nombre *</div>
              <input
                value={name}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            {type === "creativo" ? (
              <div>
                <div className="text-xs font-bold text-slate-300">Especialidad</div>
                <input
                  value={specialty}
              ...
              <div>
                <div className="text-xs font-bold text-slate-300">Empresa</div>
                <input
                  value={company}
...
//               <p className="mt-1 text-sm text-slate-300">Tus contactos guardados</p>
//             </div>
...
//               <div className="border-b border-white/8 bg-[#111827] px-6 py-5">
//                 <div className="text-sm font-extrabold text-white">Lista</div>
//                 <div className="text-xs text-slate-400">Contactos persistidos en la BD</div>
//               </div>
...
//                   <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] p-10 text-center">
//                     <div className="text-sm font-semibold text-slate-200">No hay contactos</div>
//                     <div className="mt-1 text-xs text-slate-400">Crea tu primer contacto</div>
//                   </div>
//                 ) : (
//                   <div className="grid gap-2">
//                     {filtered.map((c) => (
...
//                               <div className="mt-1 truncate text-xs text-slate-400">{c.email}</div>
...
//               <div className="border-b border-white/8 bg-[#111827] px-6 py-5">
//                 <div className="text-sm font-extrabold text-white">Detalle</div>
//                 <div className="text-xs text-slate-400">Acciones</div>
//               </div>

//               <div className="px-6 py-6">
//                 {!selected ? (
//                   <div className="text-sm text-slate-400">Selecciona un contacto.</div>
//                 ) : (
//                   <div>
//                     <div className="flex items-start gap-3">
//                       <Avatar name={selected.name} type={selected.type} />
